...
public class GameForFourController implements Game{
    ...
    public void init(){
        ...
        players.add(new GamePlayer(this,"P3"));
        setDeck();
        giveCards();
    }
    ...
    public void p2ButtonsInit(){
        ...
        players.stream()
                ...
                .get()
                .cards.stream()
                .forEach(x -> top.getChildren().add(createButton(size)));
    }

    public void p3ButtonsInit(){
        right.getChildren().clear();
        int size  = players.stream()
                .filter(player -> player.name.equals("P3"))
                .findFirst()
                .get()
                .cards.size();
        players.stream()
    ...
    public void p2Move(){
        ...
        if(card != null){
            ...
            if (isWin(players.get(2))){
                switch (card.mark){
                    case TURN: reverse(); p3Move();break;
                    case PT: players.get(3).takeTwoCards(); updateCards();; break;
                    case PF: players.get(3).takeFourCards(); updateCards();; break;
                    case PASS:  updateCards(); break;
                    default: p3Move();
                }
            }
        }else {
    ...
    public void p3Move(){
        Card card = players.get(3).putCard();
        ...
        if(card != null){
            if (card.color == Color.ALL){
                card.color = players.get(3).getColor();
            }
            currentCard = card;
            players.get(3).removeCard(card);
            updateCards();

            if (isWin(players.get(3))){
                switch (card.mark){
                    case TURN: reverse(); p2Move();break;
